                    ...
                    <div>
                        ...
                        <div class="md:grid md:grid-cols-3 md:gap-4 mx-2 my-6">
                            @foreach($students as $student)
                                <div class="card">
                                    <div>
                                        <img class="p-2 mx-auto" style="width: 230px; height:230px;" src=" {{ $student->photo }} " alt="{{ $student->name }} Image">    
                                    </div>
                                    <div class="font-medium text-xl py-1">
                                        {{ $student->name }} <br>
                                        <span class="font-normal text-lg">{{  $student->title }}</span>
                                    </div>
                                    <div class="btn-layout-control">
                                        <form action="{{ route('student.all.show', $student->id) }}">
                                        ...
                                        &nbsp;
                                        <form action="{{ route('student.all.delete', $student->id) }}" method="POST">
                                            @csrf 
                                            @method('DELETE')
