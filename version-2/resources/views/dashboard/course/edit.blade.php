            ...
            <div class="md:grid md:grid-cols-4 md:gap-4">
                <div class="col-span-4 bg-white p-3">
                    ...
                    <div class="lg:w-1/3 mx-auto">
                        <form class="bg-white shadow-lg p-4" action="{{ route('update-course', $course->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                            <div>
                                <h2 class="text-2xl text-center py-4 font-medium border-b-4 uppercase">Edit Course</h2>
                            </div>
                            <div class="my-2">
                                <img class="w-32 h-32 rounded-full p-2 mx-auto border-2" src=" {{ $course->photo }} " alt="{{ $course->name }} Image">    
                            </div>
                            <div class="my-2">
                                <input type="text" value="{{ $course->name }}" name="name" value="{{old('name')}}" placeholder="Course Name" class="input-box @error('name') border-red-500 @enderror">
                                @error('name')
                                    {{$message}}
                                @enderror
                            </div>
                            ...
                            <div class="my-2">
                                <input type="text" value="{{ $course->slug }}" name="slug" value="{{old('slug')}}" placeholder="Slug" class="input-box @error('slug') border-red-500 @enderror">
                                @error('slug')
                                    {{$message}}
                                @enderror
                            </div>
                            <div class="my-2">
                                <textarea name="description" class="px-5 w-full border border-gray-400 h-24 rounded-lg my-2 text-lg focus:outline-none @error('description') border-red-500 @enderror">{{ $course->description }}</textarea>
                                @error('description')
                                    {{$message}}
                                @enderror
                            </div>
                            ...
                            <div class="px-6 py-4 flex justify-end">
                                <button type="submit" class="btn-submit tracking-wider">Update Course</button>
                            </div>
                        </form>
                    </div>    
                </div>
